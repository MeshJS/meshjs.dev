...
```typescript
import { YaciProvider, MeshWallet, MeshTxBuilder } from "@meshsdk/core";
...
async function main() {
  ...
  const wallet = new MeshWallet({
    ...
    key: {
      type: "mnemonic",
      words: "test test test test test test test test test test test test test test test test test test test test test test test sauce".split(" "),
    },
  });
