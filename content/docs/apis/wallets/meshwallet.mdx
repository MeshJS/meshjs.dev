...
`CardanoHeadlessWallet` and `MeshCardanoHeadlessWallet` provide programmatic access to wallet functionality without requiring a browser extension. Use them for minting scripts, automated transactions, multi-signature coordination, and any scenario where your application needs to sign transactions directly.

| Class | Description |
|-------|-------------|
| `CardanoHeadlessWallet` | Low-level implementation returning raw CBOR hex |
| `MeshCardanoHeadlessWallet` | Extended class with Mesh-friendly convenience methods |
...
interface CardanoHeadlessWalletConfig {
  addressSource: AddressSource;       // Key/credential source
  networkId: number;                  // 0 = testnet, 1 = mainnet
  walletAddressType: WalletAddressType; // Base or Enterprise
  fetcher?: IFetcher;                 // For querying blockchain data
...
```tsx
// CBOR hex format (CardanoHeadlessWallet)
const balanceCbor = await wallet.getBalance();

// Mesh Asset format (MeshCardanoHeadlessWallet)
const balance = await wallet.getBalanceMesh();
```

**Response (Mesh format):**
```tsx
[
...
```tsx
// CBOR hex format (CardanoHeadlessWallet)
const utxosCbor = await wallet.getUtxos();

// Mesh UTxO format (MeshCardanoHeadlessWallet)
...
// Bech32 format (MeshCardanoHeadlessWallet)
...
```tsx
// Hex format
const usedAddressesHex = await wallet.getUsedAddresses();
...
```tsx
// Hex format
const unusedAddressesHex = await wallet.getUnusedAddresses();
...
```tsx
// Hex format
const rewardAddressesHex = await wallet.getRewardAddresses();
...
### signTxReturnFullTx (MeshCardanoHeadlessWallet)
...
```tsx
// CBOR hex format
const collateralCbor = await wallet.getCollateral();
...
### CardanoHeadlessWallet (Base Class)

| Method | Returns | Description |
|--------|---------|-------------|
| `create(config)` | `Promise<CardanoHeadlessWallet>` | Static - Create from config |
| `fromMnemonic(config)` | `Promise<CardanoHeadlessWallet>` | Static - Create from mnemonic |
| `fromBip32Root(config)` | `Promise<CardanoHeadlessWallet>` | Static - Create from bech32 key |
| `fromBip32RootHex(config)` | `Promise<CardanoHeadlessWallet>` | Static - Create from hex key |
| `fromCredentialSources(config)` | `Promise<CardanoHeadlessWallet>` | Static - Create from credentials |
...
| `getUsedAddresses()` | `Promise<string[]>` | Used addresses in hex |
| `getUnusedAddresses()` | `Promise<string[]>` | Unused addresses in hex |
| `getChangeAddress()` | `Promise<string>` | Change address in hex |
| `getRewardAddresses()` | `Promise<string[]>` | Reward addresses in hex |
...
### MeshCardanoHeadlessWallet (Extended Class)

Inherits all methods from `CardanoHeadlessWallet` plus:
...
- Which UTxOs are set as collateral (overlap possible between `getUtxos()` and `getCollateral()`)
...
- No `init()` call needed
- Methods like `getBalance()` return CBOR hex; use `getBalanceMesh()` for Mesh format
