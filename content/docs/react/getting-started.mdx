...
|----------|------|-------------|
| `wallet` | `BrowserWallet` | Wallet instance with CIP-30 methods |
...
**Solution:** The `wallet` object from `useWallet()` is a `BrowserWallet` instance. Import types from `@meshsdk/core`:
