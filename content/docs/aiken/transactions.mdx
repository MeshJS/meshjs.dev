...
```typescript
import { resolvePaymentKeyHash, BrowserWallet } from "@meshsdk/core";
...
async function lockAssets(wallet: BrowserWallet, amount: string) {
...
async function redeemAssets(wallet: BrowserWallet) {
  const { script, scriptAddress } = getScript();
  const { address, hash } = await getWalletInfo(wallet);
  const provider = new BlockfrostProvider("<YOUR_API_KEY>");
...
export async function redeemAssets(wallet: BrowserWallet): Promise<string> {
  const { script, scriptAddress } = getScript();
  const { address, hash } = await getWalletInfo(wallet);
