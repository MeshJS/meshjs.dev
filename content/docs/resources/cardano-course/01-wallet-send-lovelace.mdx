...
The `MeshWallet` class provides methods to create wallets, generate mnemonic phrases, and retrieve addresses.
...
const wallet = new MeshWallet({
  ...
  key: {
    type: "mnemonic",
    words: ["your", "24", "word", "mnemonic", "phrase", "here"],
  },
});
...
The `MeshWallet` class manages your wallet:

| Method | Description |
|--------|-------------|
| `MeshWallet.brew()` | Generates a new 24-word mnemonic |
| `getChangeAddress()` | Returns the wallet's receiving address |
| `getUtxos()` | Fetches all UTXOs owned by the wallet |
| `signTx(tx)` | Signs a transaction with the wallet's private key |
