...
The `mesh-transaction` skill includes a complete Aiken-to-MeshTxBuilder mapping guide. Give your AI assistant your Aiken type definitions and it generates the correct MeshTxBuilder transaction code — including the right data format helpers, constructor indices, and datum/redeemer conventions.

For example, given these Aiken types:
...
// Collect funds — redeemer uses Mesh format
const collectTx = await txBuilder
  ...
  .txInRedeemerValue(mConStr1([])) // VestingRedeemer::Collect (index 1)
...
The skill handles the nuances automatically: datums use JSON format with `conStr` helpers, simple redeemers use Mesh format with `mConStr` helpers, constructor indices match Aiken variant order, and script parameterization uses `applyParamsToScript` with the correct type parameter.
