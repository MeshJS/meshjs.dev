...
async function escrowDemo() {
  ...
  const partyAWallet = await BrowserWallet.enable("eternl");
  ...
  const partyBWallet = await BrowserWallet.enable("nami");
  ...
  const signedByA = await partyAWallet.signTx(completeTx, true);

  // Party B signs and submits
  const signedByBoth = await partyBWallet.signTx(signedByA, true);
