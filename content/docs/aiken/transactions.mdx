...
For the transaction, we use `spendingPlutusScriptV2()` to indicate a Plutus V2 script, `txInDatumValue` and `txInRedeemerValue` to provide the datum and redeemer, `txInScript` to provide the script, and `requiredSignerHash` to set the required signer.