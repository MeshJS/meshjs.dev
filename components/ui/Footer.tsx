    ...
    <footer className="border-t border-border bg-fd-card py-12 text-muted-foreground">
      <div className="w-full flex flex-col gap-12 items-center px-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-x-16 gap-y-20 w-full max-w-7xl">
          {linksApi.map((item) => (
            <div
              className="flex flex-col gap-3 [&>a]:hover:underline"
              key={item.link}
            >
              ...
              {"items" in item &&
                item.items.map((childItem) => (
                  <Link href={childItem.link} key={childItem.link}>
                    {childItem.title}
                  </Link>
                ))}
            </div>
          ...
          <div
            className="flex flex-col gap-3 [&>a]:hover:underline"
            key={metaSolutions.link}
          >
            <Link href={metaSolutions.link} className="font-bold">
              {metaSolutions.title.toUpperCase()}
            </Link>
            {"items" in metaSolutions &&
              metaSolutions.items.map((item) => (
          ...
          <div
            className="flex flex-col gap-3 [&>a]:hover:underline"
            key={metaResources.link}
          >
            <Link href={metaResources.link} className="font-bold">
              {metaResources.title.toUpperCase()}
            </Link>
            {"items" in metaResources &&
              metaResources.items.map((item) => (
                <Link href={item.link} key={item.link}>
                  {item.title}
                </Link>
              ))}
          ...
          <div
            ...
            key={metaGuides.link}
          >
            <Link href={metaGuides.link} className="font-bold">
              {metaGuides.title.toUpperCase()}
            </Link>
            {"items" in metaGuides &&
              metaGuides.items.map((item) => (
                <Link href={item.link} key={item.link}>
                  {item.title}
                </Link>
              ))}
          ...
          <div
            ...
            key={metaCourse.link}
          >
            <Link href={metaCourse.link} className="font-bold">
              {metaCourse.title.toUpperCase()}
            </Link>
            {"items" in metaCourse &&
              metaCourse.items.map((item) => (
                <Link href={item.link} key={item.link}>
                  {item.title}
                </Link>
              ))}
