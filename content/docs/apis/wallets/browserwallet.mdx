...
`CardanoBrowserWallet` and `MeshCardanoBrowserWallet` enable your dApp to connect with users' browser extension wallets following the [CIP-30](https://cips.cardano.org/cip/CIP-30) standard. This provides a consistent interface for wallet discovery, connection, transaction signing, and data queries across all compatible wallets.

**Supported wallets include:** Eternl, Nami, Lace, Flint, Typhon, GeroWallet, and any CIP-30 compliant wallet.

| Class | Description |
|-------|-------------|
| `CardanoBrowserWallet` | Low-level CIP-30 implementation returning raw CBOR hex |
| `MeshCardanoBrowserWallet` | Extended class with Mesh-friendly convenience methods |
...
// 3. Get wallet balance (Mesh format)
const balance = await wallet.getBalanceMesh();

// 4. Get wallet address (Bech32 format)
...
```tsx
import { CardanoBrowserWallet } from "@meshsdk/wallet";

const wallets = CardanoBrowserWallet.getInstalledWallets();
```
...
```tsx
// Raw hex format (CardanoBrowserWallet)
const changeAddressHex = await wallet.getChangeAddress();

// Bech32 format (MeshCardanoBrowserWallet)
...
```tsx
// CBOR hex format (CardanoBrowserWallet)
const balanceCbor = await wallet.getBalance();

// Mesh Asset format (MeshCardanoBrowserWallet)
const balance = await wallet.getBalanceMesh();
```
...
// Mesh UTxO format (MeshCardanoBrowserWallet)
...
### signTxReturnFullTx (MeshCardanoBrowserWallet)
...
### CardanoBrowserWallet (Base Class)

| Method | Returns | Description |
|--------|---------|-------------|
| `getInstalledWallets()` | `Wallet[]` | Static - Get installed wallets |
| `enable(walletName, extensions?)` | `Promise<ICardanoWallet>` | Static - Connect to wallet |
...
### MeshCardanoBrowserWallet (Extended Class)

Inherits all methods from `CardanoBrowserWallet` plus:
